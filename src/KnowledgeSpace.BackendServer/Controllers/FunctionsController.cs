...
namespace KnowledgeSpace.BackendServer.Controllers
{
    public class FunctionsController : BaseController
    {
        ...
        [HttpGet("filter")]
        [ClaimRequirement(FunctionCode.SYSTEM_FUNCTION, CommandCode.VIEW)]
        public async Task<IActionResult> GetFunctions(string filter = "", int pageIndex = 1, int pageSize = 10)
        {
        ...
        {
            var query = from cm in _context.Commands
                        join cif in _context.CommandInFunctions on cm.Id equals cif.CommandId into result1
                        from commandInFunctions in result1.DefaultIfEmpty()
                        select new
                        {
                            cm.Id,
                            cm.Name,
                            commandInFunctions.FunctionId
                        };
