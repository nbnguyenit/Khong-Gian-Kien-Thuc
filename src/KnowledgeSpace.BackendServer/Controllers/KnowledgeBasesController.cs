...
namespace KnowledgeSpace.BackendServer.Controllers
{
    public partial class KnowledgeBasesController : BaseController
    {
        ...
        public KnowledgeBasesController(ApplicationDbContext context
            ...
            ICacheService cacheService, IOneSignalService oneSignalService)
        {
            _context = context;
        ...
        [AllowAnonymous]
        //[ClaimRequirement(FunctionCode.CONTENT_KNOWLEDGEBASE, CommandCode.VIEW)]
        public async Task<IActionResult> GetKnowledgeBases(string filter, int? categoryId, int pageIndex, int pageSize)
        {
            /*
        ...
        public async Task<IActionResult> PostKnowledgeBase([FromForm] KnowledgeBaseCreateRequest request)
        {
            ...
            //Process label
            if (request.Labels?.Length > 0) // if (!string.IsNullOrEmpty(request.Labels))
            {
        ...
        public async Task<IActionResult> DeleteKnowledgeBase(string id)
        {
            var knowledgeBase = await _context.KnowledgeBases.FindAsync(id);
            if (knowledgeBase == null)
                return NotFound(new ApiNotFoundResponse($"Knowledge Bases with id: {id} is not found"));
            ...
            return BadRequest(new ApiBadRequestResponse("Delete Knowledge Bases failed"));
        }
